...
pod_health = 32 --stores health of target pod only (other version all pods)
pods_remaining = 5 --number of pods remaining
is_pod_health_set = 0
pod_hp_table = {}
...
pod_health_reward = 5.000001
pod_clear_reward = 50.0
boss_health_reward = 1.0
death_reward = -1.01
pod_distance_reward = 1.0
boss_clear_bonus = 50.0
...
function correct_score()
  ...
  if is_boss == 0 then
    ...
    if is_pod_health_set == 1 then 
      ...
      delta = delta + get_pod_hp_reward_target_only()
      ...
      if current_pods < pods_remaining then
        ...
        local target_key = 'pod_' .. current_target_pod
        --check that pod destroyed was the target pod
        if data[target_key] == 0 or data[target_key] > 32 then
...
function get_pod_hp_reward_target_only()
  --print("get pod hp start")
  local target_key = 'pod_' .. current_target_pod
  local current_hp = 0
  if data[target_key] > 0 and data[target_key] < 33 then
    current_hp = current_hp + data[target_key] 
  end

  r = (pod_health - current_hp) * pod_health_reward
  --if r > 0 then
    --print("getting reward > 0 for pod dmg " .. current_hp .. " " .. pod_health)
  --end
  r = clip(r,0,clip_max)
  pod_health = current_hp
  --print("get pod hp end")
